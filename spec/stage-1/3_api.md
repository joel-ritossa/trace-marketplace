...
Owner or listed. All spans for the trace ordered by `started_at`, including full `attributes` and `events` JSONB. The frontend reconstructs the tree from `source_parent_span_id`. Paginated at 500 spans per page to keep responses bounded.
...
Owner or acquirer only. Streams the original raw uploaded payload (`Content-Disposition` attachment, original filename). A listed-but-not-acquired trace returns `403 acquisition_required` with a readable message pointing at the acquire action.
