...
// Direct port of the web's components/traces/conversation.ts — keep in sync.
//
...
import type { SpanDetail, SpanEvent, SpanListItem } from "./traces";
  ...
  for (const part of dictParts(message)) {
    ...
    if (part.type === "tool_call") {
      ...
      tools.push({
        ...
        input:
          part.arguments !== undefined && part.arguments !== null ? compact(part.arguments) : null,
...
function outputMessages(detail: SpanDetail): Dict[] | null {
  return (
    ...
    eventMessages(
      detail.events,
      ["gen_ai.content.completion", "gen_ai.choice"],
      ["gen_ai.completion"],
    )
  );
}
...
export function buildConversation(
  ...
  const ordered = spans
    ...
    .filter(
      (entry): entry is { span: SpanListItem; detail: SpanDetail } => entry.detail !== undefined,
    )
    .sort(
      (a, b) =>
