...
Each call runs N times at temperature > 0 (env-tunable, default 3) and the votes fold by majority — split or abstention-majority becomes `indeterminate`, confidence = vote share, and the raw votes are stored with the result row as the audit artifact. Provider access goes through one litellm wrapper that records per-call latency, tokens, and cost into the result metadata; the judge model is an env var (default `openai/gpt-5-mini`, `services/api/app/analysis/config.py`).
