...
# Trace Marketplace Adaptation

This project is product UI, not a marketing site. Everything above is the base system; this section overrides and extends it for Trace Marketplace's surfaces (upload, trace inspection, marketplace, library). Where this section and the base disagree, this section wins.

## Marketing Chrome: Restricted
...
The base file's semantic tokens are the source; this maps them to domain states. Soft variants are backgrounds for badges/rows; deep variants are text on soft backgrounds.

| Domain state | Token mapping |
|---|---|
| Upload `received` / `processing` | `{colors.mute}` text + `{colors.canvas-soft-2}` badge; processing adds a subtle spinner, never an animated gradient |
| Upload `complete`, span/trace `ok` | `ok` green — **new tokens**: `status-ok: #29bc9b`, `status-ok-soft: #aaffec` (reuses cyan-deep/cyan-soft values; named for role) |
| Upload `failed`, span/trace `error`, error counts | `{colors.error}` / `{colors.error-soft}` / `{colors.error-deep}` |
...
Muted, consistent hues identify span kinds in the tree and detail panel — applied as a 3px left-edge bar on tree rows and a small dot on badges, never as row fills:
...
| `agent` | `{colors.link}` |
| `tool` | `{colors.warning}` |
| `retriever` | `{colors.cyan-deep}` |
| `embedding` | `{colors.highlight-magenta}` |
| `chain` / `other` | `{colors.hairline-strong}` |

These six are the only chromatic accents on app surfaces (plus the status palette). The base "don't introduce a sixth accent" rule is amended accordingly: no accents beyond this table.

## Density Rules (Inspector & Tables)

- All technical values — trace/span IDs, durations, timestamps, token counts, hashes — render in `{typography.caption-mono}` or `{typography.code}`. Names and descriptions stay in the sans.
...
Sentence-case everywhere, including buttons, empty states, and error messages. Errors are readable sentences ("You already uploaded this file."), styled with the status palette, never raw codes alone. Empty states use `ex-empty-state-card` chrome with one primary action.
