...
async def create(pool: asyncpg.Pool, consumer_id: str, trace_id: str) -> asyncpg.Record | None:
    ...
    Owners may acquire their own listed traces — the acquisition lands the
    trace in their library. The listed check lives inside the insert's select
    so an unlist can't race between a separate check and the write. Returns
    the acquisition row with a `created` flag (false = it already existed),
    or None when the trace isn't listed (and no prior acquisition exists) —
    the router maps that to 409.
    """
    return await pool.fetchrow(
        ...
        with ins as (
          ...
          select $1, $2
          from traces t
          where t.id = $2 and t.visibility = 'listed'
